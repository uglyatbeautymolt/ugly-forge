...
export default function ScrumBoard({ tasks, projects }) {
...
function Lane({ lane, tasks, projectMap }) {
  ...
  return (
    <div style={{
      ...
      {/* Cards */}
      <div style={{ padding: '8px', display: 'flex', flexDirection: 'column', gap: '6px', maxHeight: 'calc(100vh - 220px)', overflow: 'auto' }}>
        {tasks.length === 0 ? (
          <div style={{ padding: '1rem', textAlign: 'center', color: 'var(--text3)', fontSize: '12px' }}>
            Leer
          </div>
        ) : (
          tasks.map(task => (
    ...
    <div style={{
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '6px', fontFamily: 'var(--mono)', fontSize: '10px' }}>
        <span style={{ color: 'var(--text3)' }}>
          est. ${(task.cost_estimated || 0).toFixed(3)}
        </span>
        <span style={{ color: costOk ? 'var(--text2)' : 'var(--amber)' }}>
          real ${(task.cost_real || 0).toFixed(3)}
        </span>
      </div>
